...
[mariadb]
name = MariaDB
baseurl = https://mirrors.aliyun.com/mariadb/yum/10.5/centos8-aarch64/
module_hotfixes=1
gpgkey=https://yum.mariadb.org/RPM-GPG-KEY-MariaDB
gpgcheck=0
EOF"

  # 2. 安装 boost-program-options-1.66.0-13.el8.aarch64.rpm 否则会报错
  iam::common::sudo wget https://repo.almalinux.org/almalinux/8/AppStream/aarch64/os/Packages/boost-program-options-1.66.0-13.el8.aarch64.rpm
  iam::common::sudo "yum install boost-program-options-1.66.0-13.el8.aarch64.rpm"

  # 3. 安装 MariaDB 和 MariaDB 客户端
  iam::common::sudo "yum -y install MariaDB-server MariaDB-client"

  # 4. 启动 MariaDB，并设置开机启动
